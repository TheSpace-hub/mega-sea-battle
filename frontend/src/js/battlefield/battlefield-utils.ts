...
let currentMode: string = 'prepare';
...
export function initBattlefield() {
    const battlefield: HTMLElement = document.getElementById('battlefield')!;
 ...
 */
export function setMode(mode: string, player: number | null = null) {
    currentMode = mode;
...
export function updateDisplay() {
    ...
    const cells: NodeListOf<HTMLDivElement> = battlefield.querySelectorAll('.cell:not(.coordinate)');
    const modeTitle: HTMLElement = document.getElementById('current-mode')!;
    const modeDescription: HTMLElement = document.getElementById('mode-description')!;
...
function handleCellClick(cell: HTMLDivElement) {
    if (getStatus() === gameStatusTypes.WAITING_SELF_START) {
        ...
        if (getStatus() === gameStatusTypes.WAITING_SELF_MOVE) {
            const x = parseInt(<string>cell.dataset.col);
            const y = parseInt(<string>cell.dataset.row);
            attack(x, y);
        }
 ...
 */
export function addPlayerIntoBattlefields(username: string) {
    let i = 0;
    while (document.querySelector(`#mode-player-${i}`))
        i++;

    document.querySelector('#list-of-modes')?.insertAdjacentHTML(
        'beforeend', createPlayerBattlefieldItem(username, i));
 ...
 */
function createPlayerBattlefieldItem(username: string, index: number) {
    return `
 ...
 * @param field Field.
 */
function fillCell(cell: HTMLDivElement, field: any, ignoreEmpty: boolean = false) {
    const x = parseInt(<string>cell.dataset.col);
    const y = parseInt(<string>cell.dataset.row);
