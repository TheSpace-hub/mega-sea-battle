 ...
 */
export function addPlayerIntoList(username: string) {
 ...
 */
export function setPlayerStatusInList(username: string, status: string) {
    let name = '?Status?'
    if (status === 'PREPARING')
        name = 'Готовит флот'
 ...
 * @returns {string}
 */
function createItem(username: string) {
    const index = players.findIndex((player: any) => player.username === username) + 1
    return `
